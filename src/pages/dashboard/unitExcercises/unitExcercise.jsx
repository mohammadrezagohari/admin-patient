...
export function UnitExcercise() {
  ...
  const getDatas = async () => {
    const result = await getUnitExercise(userToken)
  ...
  const deleteUnitExcercise = async (id) => {
    console.log(`deleteUnits ${id}`);
    const deleteResult = await deleteUnitExercise(id, userToken)
      .then(function (response) {
        console.log(response?.data);
        setUnitExcercises(
          unitExcercises.filter((unitExcercise) => unitExcercise.id !== id)
        );
      })
      .catch(function (err) {
  ...
  useEffect(() => {
    ...
    const filtered = unitExcercises?.filter((item) =>
      item.unit.toLowerCase().includes(searchTerm.toLowerCase())
    );
    setFilteredData(filtered);
  }, [searchTerm, unitExcercises]);
  ...
  return (
    <>
      <Card>
        <div className="py-5">
          <Link
            to={`/dashboard/unitExcercises/create`}
            className="mr-3"
            style={linkStyle}
          >
            ثبت تمرین فصل جدید
          ...
          <>
            <CardBody className="min-h-screen overflow-x-scroll px-0 pt-0 pb-2">
              <table className="w-full min-w-[640px] table-auto text-right">
                ...
                <tbody ref={listRef}>
                  {displayedData?.map((unitExcercise, key) => {
                    ...
                    return (
                      <tr key={key}>
                        ...
                        <td className={className}>
                          <Typography className="text-xs font-semibold text-blue-gray-600">
                            {unitExcercise?.file_url}
                          </Typography>
                        </td>

                        <td className={className}>
                          <Typography className="text-xs font-semibold text-blue-gray-600">
                            {/* {unitExcercise?.course?.title} */}
                          </Typography>
                        </td>

                        <td className={className}>
                          <Typography className="text-xs font-semibold text-blue-gray-600">
                            {/* {unitExcercise?.grade?.name} */}
                          </Typography>
                        </td>

                        <td className={className}>
                          {console.log("unitExcercise", unitExcercise)}
                          <Link
                            to={`/dashboard/unitExcercises/show/${unitExcercise.id}`}
                            style={linkStyle}
                          >
                            اصلاح
                          </Link>
                          <Button
                            onClick={() =>
                              deleteUnitExcercise(unitExcercise.id)
                            }
